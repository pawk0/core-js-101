...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (newString.indexOf(char) === -1) return char;
  }
  return null;
}
...
function reverseInteger(num) {
  return Number.parseInt(Array.from(num.toString(10))
    .reverse()
    .join(''), 10);
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < ccnString.length; i += 1) {
    let digit = Number.parseInt(ccnString.charAt(i), 10);
...
function getDigitalRoot(num) {
  ...
  const numbers = Array.from(num.toString(10))
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openings.includes(char) && nextChar === brackets[char]) {
      return isBracketsBalanced(str.slice(0, i) + str.slice(i + 2));
    }
  }
  return false;
}
...
function getCommonDirectoryPath(pathes) {
  const newArray = pathes.map((p) => p.split('/'));

  const root = newArray[0][0];
  if (newArray.some((ar) => ar[0] !== root)) {
    return '';
  }
  const commonFolders = [];

  for (let i = 0; i < newArray[0].length; i += 1) {
    const temp = newArray.map((a) => a[i]);
    const tempSet = new Set(temp);

    if (tempSet.size === 1) {
      commonFolders.push(temp[0]);
    }
  }
